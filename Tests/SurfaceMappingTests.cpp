...
namespace
{
    ...
    MDSS::SurfaceMappingData BuildFixture(const char* FileName, std::uint32_t Resolution = 16)
    {
        const MDSS::OBJLoadResult Mesh = MDSS::OBJLoader::Load(GetMappingFixturePath(FileName));
        return MDSS::SurfaceMappingBuilder::Build(Mesh.Vertices, Mesh.Triangles, {{0, {Resolution, Resolution}}});
    }

    std::size_t CountValidTexels(const MDSS::SurfaceMappingData& Mapping)
    {
        return static_cast<std::size_t>(std::ranges::count_if(Mapping.Texels, &MDSS::SurfaceMappingTexel::IsValid));
    }

    bool HasCrossTriangleNeighbor(const MDSS::SurfaceMappingData& Mapping, bool RequireDifferentChart)
    {
        for (const MDSS::SurfaceMappingTexel& Texel : Mapping.Texels)
        {
            if (!Texel.IsValid())
            {
                continue;
            }
            for (const MDSS::LocalTexelIndex Neighbor : Texel.Neighbors)
            {
                if (Neighbor == MDSS::InvalidTexelIndex)
                {
                    continue;
                }
                const MDSS::SurfaceMappingTexel& Other = Mapping.Texels[Neighbor];
    ...
    bool HasCrossChartNeighbor(const MDSS::SurfaceMappingData& Mapping)
    {
        for (const MDSS::SurfaceMappingTexel& Texel : Mapping.Texels)
        {
            if (!Texel.IsValid())
    ...
    {
        const MDSS::OBJLoadResult Mesh = MDSS::OBJLoader::Load(GetMappingFixturePath("QuadSeam.obj"));
        Check(Mesh.Triangles.size() == 2, "seam fixture should retain two source triangles");

        const MDSS::MeshTriangleSource& A = Mesh.Triangles[0];
        const MDSS::MeshTriangleSource& B = Mesh.Triangles[1];
    ...
    {
        const MDSS::SurfaceMappingData Mapping = BuildFixture("SingleTriangle.obj", 8);
    ...
    {
        const MDSS::SurfaceMappingData Mapping = BuildFixture("QuadNoSeam.obj");
    ...
    {
        const MDSS::SurfaceMappingData Mapping = BuildFixture("QuadSeam.obj", 24);
    ...
    {
        const MDSS::SurfaceMappingData Mapping = BuildFixture("DisconnectedQuads.obj", 24);
    ...
    {
        const MDSS::SurfaceMappingData First = BuildFixture("QuadSeam.obj", 24);
        const MDSS::SurfaceMappingData Second = BuildFixture("QuadSeam.obj", 24);
        ...
        {
            const MDSS::SurfaceMappingTexel& A = First.Texels[Index];
            const MDSS::SurfaceMappingTexel& B = Second.Texels[Index];
    ...
    {
        std::vector<MDSS::Vertex> Vertices(6);
        ...
        const std::vector<MDSS::MeshTriangleSource> Triangles = {
            {{{0, 1, 2}}, {{0, 1, 2}}, {{0, 1, 2}}, 0},
            {{{3, 4, 5}}, {{3, 4, 5}}, {{3, 4, 5}}, 1},
        };
        const MDSS::SurfaceMappingData Mapping =
            MDSS::SurfaceMappingBuilder::Build(Vertices, Triangles, {{0, {8, 8}}, {1, {4, 4}}});
        ...
        for (std::size_t Index = 0; Index < Mapping.Texels.size(); ++Index)
        {
            ...
            {
                const MDSS::SurfaceLocalID ExpectedSurface = Index < 64 ? 0 : 1;
    ...
    {
        MDSS::SurfaceMappingData Mapping = BuildFixture("QuadNoSeam.obj");
        MDSS::LocalTexelIndex    Source = MDSS::InvalidTexelIndex;
        MDSS::LocalTexelIndex    Target = MDSS::InvalidTexelIndex;
        for (std::size_t Index = 0; Index < Mapping.Texels.size() && Source == MDSS::InvalidTexelIndex; ++Index)
        {
            for (const MDSS::LocalTexelIndex Neighbor : Mapping.Texels[Index].Neighbors)
            {
                if (Neighbor != MDSS::InvalidTexelIndex)
                {
                    Source = static_cast<MDSS::LocalTexelIndex>(Index);
                    Target = Neighbor;
                    break;
                }
            }
        ...
        for (MDSS::LocalTexelIndex& Neighbor : Mapping.Texels[Target].Neighbors)
        {
            if (Neighbor == Source)
            {
