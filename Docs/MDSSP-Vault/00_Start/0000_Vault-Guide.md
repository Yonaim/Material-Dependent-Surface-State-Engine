...
2. `02_Architecture/0000_Overview.md`에서 프로젝트의 전체 구성을 파악한다.
3. 공식 용어와 문서 규칙은 `01_Project-Policy/`, 결정 근거는 `03_ADR/`에서 확인한다.
4. 진행할 작업과 완료 여부는 볼트 루트의 `0000_TODO.md`에서 확인한다.

## 문서 구조

```text
프로젝트 볼트/
├── 0000_TODO.md                     프로젝트별 작업 목록
├── 00_Start/                   공통 가이드와 재사용 템플릿
│   ├── 0000_Vault-Guide.md
│   └── Templates/
├── 01_Project-Policy/          프로젝트 규칙, 공식 용어, 수식 표기 규칙
├── 02_Architecture/            전체 구조, 기능별 설계, 시스템 정의 수식
├── 03_ADR/                     설계 결정과 근거
├── 04_Development/
...
`01_Project-Policy/`에는 프로젝트에서 지킬 규칙과 용어의 공식 의미를 둔다. 수식의 기호·단위 같은 표기 규칙도 여기에 둔다. 시스템을 정의하는 수식은 `02_Architecture/`, 실제 계산 순서와 최적화는 `04_Development/Notes/`에 기록한다.
...
- 문서·폴더 이름을 바꾸면 Obsidian 내부 링크도 함께 확인한다. 완료 여부는 구현이나 실험 결과를 확인한 뒤 `0000_TODO.md`에 반영한다.
