...
- Shared Surface Geometry Data는 유효성, Normal, Meso Virtual Height, Curvature/Concavity 등 정적 Geometry 값과 Neighbor, Distance, Height Difference, Boundary, UV seam 연결 등 texel 관계 정보를 담는다.
...
- State, Overflow, `inputFactor`, `transferRate`, `decayRate` 등의 Profile 반응 파라미터는 `.Surface`에 저장하지 않는다. 반응 파라미터는 `.SRProfile`, 동적 State/Overflow는 `SurfaceInstanceStateData`가 소유한다.
...
`.Surface`는 현재 구현에서 version 1 binary cache로 직렬화하며, 필드는 명시적인 little-endian 정수·float encoding으로 기록한다. 입력 fingerprint는 cache invalidation 목적의 64-bit FNV-1a이며 보안용 hash가 아니다. GPU 업로드 표현은 별도 설계에서 정한다.
